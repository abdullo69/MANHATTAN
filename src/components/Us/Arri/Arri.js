...
function Arri() {
    ...
    useEffect(() => {
        const getData = async () => {
            let box = []
            let FirebaseData = await getDocs(collection(db, "yangi"))
    ...
    const deleteBlog = async (docs) => {
        ...
        await deleteDoc(doc(db, "yangi", docs.id));
        window.location.reload()
    }
    const titleUpdate = async (id) => {

        const Update = doc(db, "yangi", id);
    ...
    return (
        ...
        <div className='Barri'>
            ...
            >
                {data.map((doc, inx) => (
                    <SwiperSlide className='Barri_card' key={inx}>
