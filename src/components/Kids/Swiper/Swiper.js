...
export default function App() {
    ...
    useEffect(() => {
        const getData = async () => {
            let box = []
            let FirebaseData = await getDocs(collection(db, "kid"))
    ...
    const deleteBlog = async (docs) => {
        ...
        await deleteDoc(doc(db, "kid", docs.id));
        window.location.reload()
    }
    const titleUpdate = async (id) => {

        const Update = doc(db, "kid", id);
    ...
    return (
        <>
            ...
            >
                {data.map((doc, inx) => (
                    <SwiperSlide className='arri_card' key={inx}>
